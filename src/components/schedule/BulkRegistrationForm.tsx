...
export default function BulkRegistrationForm({ storeId, existingShifts, onDone }: Props) {
  ...
  const selectAll = () => {
    if (selectedUserIds.length === employees.length) {
      ...
      setSelectedUserIds(employees.map((e) => e.user_id));
    }
  };
  ...
  const totalEntries = selectedUserIds.length * dates.length;
  ...
  const isValid = selectedUserIds.length > 0 && dates.length > 0;
        ...
        <div className="grid grid-cols-2 gap-2 max-h-[160px] overflow-y-auto border rounded-md p-2">
          {employees.map((e) => (
            <label key={e.user_id} className="flex items-center gap-2 text-sm cursor-pointer hover:bg-accent/30 rounded px-1 py-0.5">
              ...
              {e.position && <span className="text-[10px] text-muted-foreground">({e.position})</span>}
            </label>
          ))}
        </div>
        {selectedUserIds.length > 0 && (
          <p className="text-xs text-muted-foreground mt-1">{selectedUserIds.length}명 선택됨</p>
        )}
      </div>

      {/* Date range */}
      <div className="grid grid-cols-2 gap-3">
        <div><Label className="flex items-center gap-1"><Calendar className="w-3 h-3" />시작일</Label><Input type="date" value={dateStart} onChange={(e) => { setDateStart(e.target.value); if (!dateEnd) setDateEnd(e.target.value); }} /></div>
        ...
        <Card className="bg-muted/50">
          <CardContent className="p-3 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {selectedUserIds.length}명 × {dates.length}일 = <strong className="text-foreground">{totalEntries}건</strong>
            </p>
            <p className="text-xs text-muted-foreground">
