...
BEGIN
  ...
  FOREACH uid IN ARRAY user_ids LOOP
    ...
    VALUES (uid, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
            ...
            jsonb_build_object('full_name','rls-test','role','part_time'));
  END LOOP;

  -- Wipe rows the handle_new_user trigger auto-created for these users
  SELECT array_agg(DISTINCT organization_id) INTO auto_org_ids
    FROM public.employee_profiles WHERE user_id = ANY(user_ids);
  SELECT array_agg(DISTINCT brand_id) INTO auto_brand_ids
    FROM public.employee_profiles WHERE user_id = ANY(user_ids);
  SELECT array_agg(DISTINCT store_id) INTO auto_store_ids
    FROM public.employee_profiles WHERE user_id = ANY(user_ids);
  ...
  IF auto_store_ids IS NOT NULL THEN
    DELETE FROM public.stores WHERE id = ANY(auto_store_ids);
  END IF;
  IF auto_brand_ids IS NOT NULL THEN
    DELETE FROM public.brands WHERE id = ANY(auto_brand_ids);
  END IF;
  IF auto_org_ids IS NOT NULL THEN
    DELETE FROM public.organizations WHERE id = ANY(auto_org_ids);
  END IF;
...
DECLARE
  ...
  auto_store_ids uuid[];
BEGIN
  -- Capture trigger-created scaffolding tied to these users
  SELECT array_agg(DISTINCT organization_id) INTO auto_org_ids
    FROM public.employee_profiles WHERE user_id = ANY(user_ids);
  SELECT array_agg(DISTINCT brand_id) INTO auto_brand_ids
    FROM public.employee_profiles WHERE user_id = ANY(user_ids);
  ...
  DELETE FROM public.company_settings WHERE organization_id IN (org_a, org_b)
    OR (auto_org_ids IS NOT NULL AND organization_id = ANY(auto_org_ids));
  DELETE FROM public.user_consents WHERE user_id = ANY(user_ids);
  DELETE FROM public.employee_profiles WHERE user_id = ANY(user_ids);
  DELETE FROM public.user_store_roles WHERE user_id = ANY(user_ids);
